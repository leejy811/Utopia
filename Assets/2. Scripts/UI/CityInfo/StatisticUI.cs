...
public class StatisticUI : MonoBehaviour, IObserver
{
    ...
    public void SetValue()
    {
        ...
        happinessText.text = ((int)RoutineManager.instance.cityHappiness).ToString() + "%<size=8>(" + Mathf.Abs((int)RoutineManager.instance.cityHappinessDifference).ToString() + "% " + (RoutineManager.instance.cityHappinessDifference > 0 ? "증가" : "감소") + ")</size>";
        residentText.text = ResidentialBuilding.cityResident.ToString() + "명<size=8>(" + Mathf.Abs(ResidentialBuilding.cityResident - ResidentialBuilding.yesterDayResident).ToString() + "명 " + ((ResidentialBuilding.cityResident - ResidentialBuilding.yesterDayResident) > 0 ? "증가" : "감소") + ")</size>";
